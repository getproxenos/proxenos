...
- **Prompt assembly / cache-breakpoint allocation.** *Mostly resolved for v0.* Prompt producers register prompt declarations once; per-request rendering is lean and host-planned. Ordering is slot + weight + anchors, and cache breakpoints are applied by a single host policy layer to the last non-empty message in eligible groups, capped by provider limits. Remaining gaps: context-grant vocabulary, render failure semantics, token-budget reporting, and provider-specific cache adapters. See `design-notes/prompt-declaration-walkthrough.md`.
